...
export async function POST(request: NextRequest) {
  try {
    console.log('[API] Creating test04 data...')
    ...
    return NextResponse.json({ 
      success: true, 
      message: 'Test04 data created successfully!',
      websiteId: websiteRef.id,
      clonedSiteId: clonedSiteRef.id
    })
    
  } catch (error) {
    console.error('[API] Error creating test04 data:', error)
    return NextResponse.json({ 
      success: false, 
      error: 'Failed to create test04 data' 
    }, { status: 500 })
  }
}
