...
interface ClonedSiteRegisterFormProps {
  siteId: string
  siteName: string
  onRegister: (userData: RegisterData) => Promise<{ success: boolean; message: string }>
  onGoogleRegister?: () => Promise<{ success: boolean; message: string }>
  onSuccess: () => void
  isFirstUser?: boolean
}

export default function ClonedSiteRegisterForm({ siteId, siteName, onRegister, onGoogleRegister, onSuccess, isFirstUser = false }: ClonedSiteRegisterFormProps) {
  const [formData, setFormData] = useState<RegisterData>({
    ...
    confirmPassword: "",
    phone: "",
    role: "member", // Default role, will be overridden by system
    inviteCode: "",
  })
    ...
    <div className="bg-gradient-to-br from-gray-900/50 to-gray-800/30 backdrop-blur-sm border border-gray-700/50 rounded-xl p-8 max-w-md w-full">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-sm text-gray-500">
          มีบัญชีอยู่แล้ว?{" "}
          <Link href={`/${siteId}/login`} className="text-blue-400 hover:text-blue-300 transition-colors">
            เข้าสู่ระบบ
          </Link>
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* Invite Code */}
        <div>
          <label htmlFor="inviteCode" className="block text-gray-300 text-sm font-medium mb-2">
            รหัสเชิญ {!isFirstUser && <span className="text-red-400">*</span>}
            {isFirstUser && <span className="text-green-400 text-xs ml-2">(ไม่บังคับสำหรับผู้สร้างเว็บครั้งแรก)</span>}
          </label>
          <input
            type="text"
            id="inviteCode"
            ...
            onChange={handleInputChange}
            placeholder={isFirstUser ? "กรอกรหัสเชิญ (ไม่บังคับ)" : "กรอกรหัสเชิญจาก Admin"}
          ...
          {isFirstUser && (
            <p className="text-xs text-green-400 mt-1">
              💡 คุณเป็นผู้สร้างเว็บโคลนครั้งแรก จะได้รับสิทธิ์ Admin อัตโนมัติ
            </p>
          )}
        </div>
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="w-full bg-gradient-to-r from-blue-600 to-blue-700 text-white font-medium py-3 px-6 rounded-xl hover:from-blue-700 hover:to-blue-800 transition-all duration-300 hover:scale-105 hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
        >
